...
RUN curl https://rclone.org/install.sh | bash && \
    wget https://git.io/gclone.sh && bash gclone.sh && \
...
CMD /bin/bash -c "envsubst '\$PORT' < /etc/nginx/conf.d/default.conf.template > /etc/nginx/conf.d/default.conf" && nginx -g 'daemon on;' && --webui-port=8080 && cd /usr/src/app && mkdir Downloads && bash start.sh
